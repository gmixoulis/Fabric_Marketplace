...
describe('Chaincode', () => {
    ...
    beforeEach('Sandbox creation', async() => {
        ...
        await token.Initialize(ctx,'some name','some symbol');
    ...
    describe('#MintWithTokenURI', () => {
        it('should work with a new token', async () => {
            ...
            sinon.assert.calledWith(mockStub.putState.getCall(2), 'nft_101', Buffer.from(JSON.stringify(nft)));
            sinon.assert.calledWith(mockStub.putState.getCall(3), 'balance_Alice_101', Buffer.from('\u0000'));
