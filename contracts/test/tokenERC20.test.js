...
describe('Chaincode', () => {
    ...
    describe('#_transfer', () => {
        ...
        it('should transfer to a new account when the sender has enough token', async () => {
            ...
            sinon.assert.calledWith(mockStub.putState.getCall(3), 'balance_Alice', Buffer.from('0'));
            sinon.assert.calledWith(mockStub.putState.getCall(4), 'balance_Bob', Buffer.from('1000'));
        ...
        it('should transfer to the existing account when the sender has enough token', async () => {
            ...
            const response = await token._transfer(ctx, 'Alice', 'Bob', '1000');
            sinon.assert.calledWith(mockStub.putState.getCall(3), 'balance_Alice', Buffer.from('0'));
            sinon.assert.calledWith(mockStub.putState.getCall(4), 'balance_Bob', Buffer.from('3000'));
            expect(response).to.equals(true);
        });
    ...
    describe('#Initialize', () => {
        ...
        it('should failed if called a second time', async () => {
            // We consider it has already been initialized in the before-each statement
            await expect(token.Initialize(ctx, 'some name', 'some symbol', '2'))
    ...
    describe('#Mint', () => {
        it('should add token to a new account and a new total supply', async () => {
            ...
            sinon.assert.calledWith(mockStub.putState.getCall(3), 'balance_Alice', Buffer.from('1000'));
            sinon.assert.calledWith(mockStub.putState.getCall(4), 'totalSupply', Buffer.from('1000'));
            expect(response).to.equals(true);
        });
        ...
        it('should add token to the existing account and the existing total supply', async () => {
            ...
            const response = await token.Mint(ctx, '1000');
            sinon.assert.calledWith(mockStub.putState.getCall(3), 'balance_Alice', Buffer.from('2000'));
            sinon.assert.calledWith(mockStub.putState.getCall(4), 'totalSupply', Buffer.from('3000'));
            expect(response).to.equals(true);
        });
