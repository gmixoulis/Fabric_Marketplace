...
class TokenERC20Contract extends Contract {
    ...
    async _spendAllowance(ctx, owner, spender, value) {
        const currentAllowance = this.Allowance(ctx, owner, spender);
        if (currentAllowance !== Number.MAX_SAFE_INTEGER) {
            ...
            const allowanceBytes = await ctx.stub.getState(allowanceKey);
            this.allowances[owner][spender] = currentAllowance - value;
        }
    }
