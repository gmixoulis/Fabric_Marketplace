...
class FractionTokenContract extends TokenERC721Contract {
    ...
    async countFractionalTokens(ctx, originalTokenId) {
        let fractionCounter = 0;
    
        try {
            const fractionIterator = await ctx.stub.getStateByPartialCompositeKey(fractionPrefix, [originalTokenId]);
            
            while (true) {
                const response = await fractionIterator.next();
                if (response.value && response.value.key) {
                    // Increment the counter for each fractional token found
                    fractionCounter++;
                } else {
                    // No more fractions to iterate
                    break;
                }
            }
    
            await fractionIterator.close();
        } catch (error) {
            console.error(`Error in countFractionalTokens: ${error.message}`);
            throw new Error(`Failed to count fractional tokens for ${originalTokenId}: ${error.message}`);
        }
    
        console.info(`Total fractional tokens found for ${originalTokenId}: ${fractionCounter}`);
